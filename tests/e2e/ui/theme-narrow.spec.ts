...
test('窄屏面板为紧凑态，单一对话前景进入 focus（W7）', async ({ page }) => {
...
test('计划页提供筛选，更多菜单只承载通用操作并在 Esc 后恢复焦点', async ({ page }) => {
  ...
  await expect(page.getByRole('menuitem', { name: '目标与里程碑' })).toHaveCount(0)
  await expect(page.getByRole('menuitem', { name: '今日进展' })).toHaveCount(0)
