...
interface Mounted {
  ...
  get(): Promise<{ status: number; body: { current: string; update?: { latest: string; tag: string }; checkedAt?: number } }>
}
...
const NEWER: VersionTags = { latest: '0.4.0-rc5', rc: '0.5.0-rc.2', beta: '0.5.0-beta.3' }
...
describe('GET /yolo/version', () => {
  ...
  it('serves the cached answer, and only that', async () => {
    ...
    expect(first.body).toEqual({ current: '0.5.0-rc.1', update: { latest: '0.5.0-rc.2', tag: 'rc' }, checkedAt: 1_000 })

    await get()
    expect(fetchTags).toHaveBeenCalledTimes(1)
  })

  it('reports no update when the running version is the newest published', async () => {
    const { get, handle } = mount({ current: '0.5.0-rc.1', fetchTags: async () => ({ rc: '0.5.0-rc.1' }) })
    await handle.refresh()
  ...
  it('stays silent when the registry read fails', async () => {
    ...
    // which is exactly what an offline host should look like.
  ...
  it('shares one in-flight read between concurrent refreshes', async () => {
    ...
    await refresh
    expect(fetchTags).toHaveBeenCalledTimes(1)
    expect(mounted.handle.snapshot().update).toEqual({ latest: '0.6.0', tag: 'rc' })
  })
})
