...
function emitUserMessage(onEvent: Handler, text: string, session = { header: { id: 'session-1', cwd } }): void {
  ...
  onEvent(session, { type: 'user/message', data: { content: [{ type: 'text', text }], source: { kind: 'user' } } })
}

/** A user-ROLE message the host authored, not the human (dsh 0.1.5 model-switch notice). */
function emitPluginNotice(onEvent: Handler, text: string, session = { header: { id: 'session-1', cwd } }): void {
  onEvent(session, {
    type: 'user/message',
    data: { content: [{ type: 'text', text }], source: { kind: 'plugin', plugin: 'model-selection', form: 'notice' } },
  })
}
...
describe('memory apply()', () => {
  ...
  // notice on the user ROLE but with `source.kind === 'plugin'`. It must not
  // become the tracked user message or drive a recall prewarm.
  it('ignores a plugin-authored user-role notice', () => {
    const { ctx, handlers, contexts } = makeCtx()
    apply(ctx as never)
    const onEvent = handlers.get('session/event')!

    yolo.addTodo(cwd, { title: '准备季度汇报材料', source: 'manual' })
    emitPluginNotice(onEvent, '[model changed: assistant turns above this point were generated by a; the session continues with b]')
    expect(contexts[0].text()).toBe('')

    // A real human message afterwards still drives recall normally.
    emitUserMessage(onEvent, '季度汇报')
