...
退出码 2 结束。门禁要求 prediction 完整且来自单一路由、每层至少 6 条、阈值（默认 0.85）自动候选中的 false-link 为 0、总体 exact
不低于 0.8、missed-link 不高于 0.15、阈值高置信自动候选无误授权且覆盖至少一半安全 LINK/UPDATE。
这套对抗式语料覆盖接近真实的身份冲突，可支撑默认关闭的实验入口，但不单独构成默认开启 R2a 的授权。
独立真实对话不作为近期硬门槛；用户反馈不准或模型、prompt、阈值、授权范围变化时，再从脱敏 shadow 日志
扩充并独立标注本评测集。
