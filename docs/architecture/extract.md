...
等高重合改写交给 resolver 做语义判断。fallback 候选带 `match_source=similarity`；即使 resolver 返回高置信，
...
只有显式开启后，单一 resolver 结果、单一开放 canonical 候选、置信度至少达到阈值
（`extraction.todoIdentityR2MinConfidence`，默认 `0.85`）且抽取形状无歧义时，
...
- `extraction.todoIdentityR2MinConfidence`：R2a 自动关联置信度阈值，默认 `0.85`，取值 `0..1`；resolver 提示词
  也会按该阈值重新锚定“确定 / 有歧义”的置信度区间，避免歧义样本越过更低阈值被误授权。
