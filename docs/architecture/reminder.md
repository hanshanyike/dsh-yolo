...
该插件扫描所有已登记工作区的到期 todo，生成通知投递记录并最佳努力投递到对应工作区的 YOLO 常驻
...
通知投递与审计写入是保底结果；resident thread 创建或 `followup` 失败不能撤销记录，也不能使
...
一样只进入通知记录与 YOLO 常驻线程。通知投递的 `seen_at` 与提醒领域的 `handled_at` 相互独立：阅读
简报或打开通知记录不会完成、推迟或处理事项。

早报正文先按“逾期 → 今日到期 → 优先级 → 到期时间”选出一项优先处理，再展示今日到期、逾期、
昨日新增未完成和目标变化；长列表只展示前三项并标注剩余数量。晚报展示今日完成、今日新增、未完成
总数（含逾期数）和明日优先事项。通知标题单独呈现，正文不重复标题；简报正文不做两行截断。
