...
- **作用域键** = `sha1(cwd)/<git-branch>`——每个工作区和分支的组合对应一个数据库。
...
| 聚合读取，写入固定到工作区 | 用户看到一份跨工作区计划；同时将 `scope_cwd` 限制在注册表内并固定到原分支作用域，确保动作安全 |
| 以 SQLite 为事实源、Markdown 为审阅投影 | SQLite 承担查询与事务；快照便于 git 比较和人工审阅，但当前不承担数据库恢复 |
| 按工作区 + 分支划分作用域 | 项目与实验彼此隔离；分支作用域键使长期分支拥有独立记忆上下文 |
