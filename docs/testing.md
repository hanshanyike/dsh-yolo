...
| W14 | 提醒与可访问性 | 新通知显示右下角非模态提示，历史通知不补弹、多条不堆叠；首页按事项聚合，badge 精确表示未读投递，通知记录完整分页且不复制事项动作；前景不被新通知抢占，所有按钮、焦点陷阱、焦点返回和 live region 可用；插件设置页的 YOLO 配置卡与宿主自带插件卡同构（同一列表、默认折叠、相同外壳与保存语义），每个控件都有可读名 |
