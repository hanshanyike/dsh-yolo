...
`POST /yolo/notifications/seen`、`POST /yolo/notifications/dismiss`、`POST /yolo/actions` 等端点，
验证持久化、作用域和载荷契约。修改 `src/**` 或共享 payload 后优先运行此层。
...
| W2 | 页面导航与计划筛选 | 首页、计划、目标、历史四入口，计划内“全部 / 今天 / 接下来 / 未排期”和历史内“按时间 / 按事项”一致；四个计划分段是开放事项的完整划分（今天＝逾期＋今天到期、接下来＝有日期、未排期＝无日期，全部默认落地并按分段分组展示），无日期事项行内显示“未排期”；标准 Tab 键盘导航、计数、空态和跨页去重正确；没有“进展”或“Agent 任务”一级入口 |
| W3 | 事项处理 | 首页、计划和历史中的完成、推迟、再提醒、编辑、取消与重新打开可用；“按日期删除事项”可预览范围并区分可恢复批量取消和强确认永久删除；变化同步到所有页面，服务端撤销恢复原值 |
...
| W14 | 提醒与可访问性 | 新通知显示右下角非模态提示，历史通知不补弹、多条不堆叠；首页按事项聚合，badge 精确表示未读投递，通知记录完整分页且不复制事项动作；单条 `×` 与「一键清除」删除的是投递记录本身（跨工作区、刷新后不复活、不影响事项状态），清除前有可取消的确认；前景不被新通知抢占，所有按钮、焦点陷阱、焦点返回和 live region 可用；插件设置页的 YOLO 配置卡与宿主自带插件卡同构（同一列表、默认折叠、相同外壳与保存语义），每个控件都有可读名 |
