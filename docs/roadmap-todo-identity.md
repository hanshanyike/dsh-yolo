...
- [x] 保持显式 id / source fingerprint 的确定性幂等链路；只把经指标批准的同 scope 新提及自动关联。
- [x] 后续提及写 evidence；字段变化按稳定 id 走领域动作，禁止依赖标题静默选中多个候选之一。
- [x] 多候选、终态语义不明和助手自行拆出的顶层事项进入 `ASK`，不自动修改。
- [x] R2c 在事项详情展示自动关联回执；用户可标记“不是同一事项 / 不该修改时间 / 其他原因”。
  纠错追加审计记录并让错误来源退出有效投影；只有未被后续编辑覆盖的自动改期才安全撤销。

### R3：状态感知合并确认与撤销

- [x] 在当前“目标状态权威”的显式终态合并基础上，加入一开一终态、两终态冲突的用户确认与结果选择。
- [x] 合并迁移待投递提醒、未处理通知、历史别名和来源关系；历史显示“B 已并入 A”，不显示为用户取消。
- [x] 增加可审计的撤销合并；撤销后恢复来源关系和业务状态，不篡改旧事件，也不覆盖合并后的用户编辑。
- [x] 助手看板设置提供默认关闭的 R3 实验开关；关闭时不投影候选，开启后也只建议，不自动合并。
- [x] 候选发现复用 resolver 的模型语义 LINK/UPDATE 观察，并以受保护的标题相似度覆盖手动事项；详情展示
  推荐理由和置信度，“不是重复事项”反馈会持久抑制同一候选对。相似度召回只扩大 R3 建议，不能授权 R2a 自动写入。
