...
1. **中间层叫什么**：`计划`（推荐，中文语感准且页面名可复用）／`项目`（与业界字面一致）。
2. **一级入口怎么排**：5 个（首页/事项/计划/目标/历史，推荐）还是 4 个（把事项时间视图塞回计划页）。
...
| # | 问题 | 选项 | 我的建议 |
|---|---|---|---|
| 1 | 中间层叫什么 | `计划` / `项目` | **计划**：中文里"发布计划""这周的计划"都指一个有终点的单元；且能接管现有页面名 |
| 2 | 一级入口 | 5 个 / 4 个 | **5 个**：三类对象各一页，避免再次出现"两排相似控件" |
| 3 | 数据迁移 | 启发式分诊 + 人工确认 / 只做保守映射 | **分诊 + 确认**：否则真实的 4 条里程碑会永远停在错误形态 |
| 4 | 是否现在就动手 | 从 P0 开始 / 先只更新文档 | 从 **P0** 开始：后端先行、UI 不动，风险最小且能立刻验证模型 |
