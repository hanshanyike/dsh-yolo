...
- **Client bundle discovery — CORRECTED (2026-08-21)**: the earlier "dev-mode client bundle NOT injected" conclusion was **WRONG**. `ClientModuleRegistry` (host/packages/client/modules/src/index.ts) scans loader entries at startup and calls `require.resolve('<entry.name>/package.json')`; if the manifest declares `dsh.client: { platform: 'web' }` (an OBJECT — a string is rejected by `parseDshClient`) it serves `exports['./client']`. Three conditions must hold: (1) entry names must resolve to the package (`dsh-plugin-yolo/dist/src/...` subpaths + a bare `dsh-plugin-yolo` entry, with a `~/.dsh/profiles/node_modules/dsh-plugin-yolo` junction), (2) `dsh.client` is an object, (3) the client bundle is CJS wrapped in `window.__ModuleLoader__.load({id, factory})` + a `process` shim (React CJS entry) — `scripts/wrap-client.mjs` post-build. `scripts/dev.mjs` generates the runtime patch (`cordis.dev.local.yml`) with package-name entries.
- **client bundle build contract**: served as a classic `<script>` → must be CJS (`module.exports`; ESM `export {}` leaves the factory with an empty exports → "loaded without registering"), no bare Node globals (`process is not defined`). Verified in a `new Function()` sandbox: `load({id:"dsh-plugin-yolo"})` → factory returns `{apply, inject: ["slots","conversationEvents","conversationViews"], name:"yolo-client"}`.
