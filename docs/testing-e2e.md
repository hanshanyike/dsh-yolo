...
| `ui/home-plan-history.spec.ts` | 首页安静/正常/高压；计划“全部/今天/接下来/未排期”的完整划分、无日期行显示“未排期”与 全部 分组；目标独立入口；历史“按时间/按事项”、状态筛选与事项展开；没有“进展”或“Agent 任务”入口 | W2 / W8 / W11 / W15 / HOME / HIST |
...
| `ui/milestone-overlap.spec.ts` | 两条无日期 + 一条近期日期里程碑（直写工作区 SQLite 播种）在目标轨道上圆点互不重合、标签互不压字，每枚圆点仍可独立点击打开编辑弹层 | W2 / W7 |
| `ui/milestone-ownership.spec.ts` | 目标卡片只列自己承载的里程碑（`goal_milestones` 播种），别的目标/别的库的里程碑不再出现在它的卡片里；无归属的开放里程碑留在「其他里程碑」轴上仍可见可编辑，目标自有里程碑的编辑器在卡片内打开 | W2 / W7 |
...
| VC-01 | unit + api + ui | 版本比较按 SemVer 优先级（预发布低于同号正式版、数字段按数值比较、非法版本不参与），且只认 npm 的 `latest`；`/yolo/version` 只回答宿主缓存结论且不等待网络，更新必须是严格更新的版本；查不到与「已是最新」在 UI 上同形（无提示位）；有新版本时折叠状态就能看到标题标记，展开后有版本号与可直接执行的更新命令 |
...
- 纯日期在本地当日 `00:00` 起进入全天提醒窗口；`reminder.aheadMin=0` 时，通知卡应在当天
  开始后的一个提醒扫描周期内产生——「今天要出报告」从当天一早即可提醒，而不是拖到日终。
  逾期判断仍以当日结束为界：当天不逾期、次日才逾期。`aheadMin` 可让纯日期提前到前一晚触发。
  仓库没有独立的“白天默认钟点”设置，纯日期就是全天窗口；09:00 早报也不承担这个语义。
...
|---|---|---|---|
| RM-DIALOG-02 | 我今天要出一份 dsh-yolo 分析报告 | 新增开放 todo“出一份 dsh-yolo 分析报告”，`due_at=2026-08-26` | 当天 00:00 前不提醒；当天开始后的一个扫描周期内恰好一张通知卡 |
| RM-DIALOG-03 | 明天把客户访谈纪要发给产品组 | 新增 todo，`due_at=2026-08-27` | 27 日当天（00:00 起）提醒，不在 26 日误发 |
| RM-DIALOG-04 | 周五下班前把报销单交了 | 新增 todo，`due_at=2026-08-28`；“下班前”没有可验证钟点时不得臆造具体时间 | 周五当天提醒；若将来由工作时段配置解析钟点，必须另立契约与用例 |
| RM-DIALOG-05 | 下周一提交 0.3.1 的发布说明 | 新增 todo，`due_at=2026-08-31` | 31 日当天提醒，不能落到本周一或下周日 |
...
| RM-DIALOG-10 | 9 月 1 日把合同盖章件寄给客户 | 新增 todo，`due_at=2026-09-01` | 9 月 1 日当天提醒，月界不能解析成 8 月 1 日 |
...
| RM-DIALOG-28 | `2026-08-27 00:02 +08:00`；同一句“今天把值班记录发给组长” | `due_at=2026-08-27` | 27 日当天窗口触发，不沿用前一自然日 |
...
当天窗口加扫描延迟内产生且只产生一张通知卡。任何维度缺测或只能从助手回复推断结果，都应判为
“不充足”。

纯日期提醒契约已实施：无钟点事项从当天 `00:00` 起进入提醒窗口（不再拖到日终），逾期判断仍以
当日结束为界。若要改成“晚间某个更早时段才提醒”，需要另立默认钟点、已过默认钟点时的回退，以及
与安静时段/早晚报的关系，并同步架构契约、配置、单测和本用例库，不能只改模型提示词。
...
- [ ] 🤖 **W2 一级导航**：首页 / 计划 / 目标 / 历史支持下划线指示、roving tabindex 与 Left/Right/Home/End；没有“进展”或“Agent 任务”一级入口
- [ ] 🤖 **W2 计划分段**：全部 / 今天 / 接下来 / 未排期 是开放事项的完整划分（今天＝逾期＋今天到期、接下来＝有日期、未排期＝无日期），全部 默认落地并按分段分组；高级筛选、时段 chip 与清除行为一致；首页不复用计划筛选状态
