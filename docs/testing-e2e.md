...
| VC-01 | unit + api + ui | 版本比较按 SemVer 优先级（预发布低于同号正式版、数字段按数值比较、非法版本不参与）；`/yolo/version` 只回答宿主缓存结论且不等待网络，更新必须是严格更新的版本与非空通道；离线/失败与「已是最新」在 UI 上同形（无提示位）；有新版本时折叠状态就能看到标题标记，展开后有版本、通道与可直接执行的更新命令 |
