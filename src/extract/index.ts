...
function mergeExtraction(yolo: Yolo, cwd: string, r: ExtractionResult): void {
  ...
  for (const t of r.todos) yolo.addTodo(cwd, { title: t.title, due_at: t.due_at, priority: t.priority as never, source: 'llm' })
...
export function apply(ctx: Context): void {
  ...
  ctx.on('session/event', (session: Session, event: { type: string; data: unknown }) => {
    ...
    const text = contentBlocksToText((event.data as { content?: never[] }).content)
    if (!text) return
