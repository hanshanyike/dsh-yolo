...
- 远程仓库长期只保留 `develop` 与 `main` 两个分支；功能、修复、试验和 agent 工作分支只保留在本地，
  **不得推送到远程**。
- `develop` 是日常开发与集成分支。功能分支完成并通过适用门禁后，先合入本地 `develop`，再只推送
  `develop` 到 `origin`；不能用远程功能分支或 PR 代替本地集成。
- `main` 是稳定分支。只有用户明确授权发布或稳定晋级时，才把已验证的 `develop` 合入 `main` 并推送；
  历史发布、旧 CI 结果或一次功能完成都不构成更新 `main` 的授权。
- 若远程出现 `develop` / `main` 之外的分支，在其提交已经安全进入 `develop` 后删除远程分支，并用
  `git ls-remote --heads origin` 核对远程最终只剩这两个分支。
...
- `0.x` 阶段：向后兼容的修复和新增能力均按发布批次提升 PATCH，不按单个功能计数；破坏性 API / 配置 /
  存储 / 行为变化或明确的大阶段切换才提升 MINOR，并记录迁移。`1.0.0` 以后恢复标准 SemVer：兼容功能
  提升 MINOR、破坏性变化提升 MAJOR。纯文档、测试或内部重构随下一次适用发布归档，不单独为其 bump。
