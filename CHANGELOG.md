...
- 新增默认关闭的“高置信事项自动关联”实验能力：仅对唯一开放候选执行置信度至少 0.98 的稳定 ID LINK 或明确截止时间 UPDATE；设置页可显式启用，其他字段、终态、多候选、周期和步骤仍保持不变。
- 事项详情新增自动关联回执与纠错：展示原话、置信度和处理结果，可反馈“不是同一事项 / 不该修改时间 / 其他原因”；原始 resolver/evidence 保留，错误来源退出有效投影，未被后续编辑覆盖的自动改期可安全撤销。
- 新增默认关闭的 R3 重复事项合并建议：复用 resolver 模型语义判断并以受保护的标题相似度覆盖手动事项，展示推荐理由和置信度；用户可标记“不是重复事项”，或预览两侧字段、选择保留项后确认合并。
- R3 合并新增可审计撤销：未处理通知、待投递提醒、历史别名和来源关系随保留项迁移；撤销恢复原关系和业务状态，不改写旧事件，也不覆盖合并后的用户编辑。

### Changed

- 事项身份 resolver 增加宿主当前模型配置回放、分层评测和 engineering gate；语义相似候选只扩大人工确认建议，不能绕过 R2a 自动写入安全门。
...
- 领域事件新增稳定主体、标题快照和结构化字段前后值；事项改名后历史不断链，同名与跨工作区事项不会按标题误合并。
- 新增按截止日期或创建日期预览事项的数据管理入口；可批量取消开放事项，也可在强确认后永久删除范围内事项及直接关联投影。
...
- “已结束”从历史展示模式调整为按事项的状态筛选；旧事件缺少稳定主体时只在时间线显示，不进行不可靠回填。
- 取消继续保留审计并支持重新打开；永久删除与取消明确分离，且不改写原始宿主会话或既有时间线。
